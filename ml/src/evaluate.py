        ...
        with engine.connect() as conn:
            with conn.begin():
                ...
                prod_row = conn.execute(
                    text("SELECT mv.id, mv.version_string, mm.test_accuracy, mm.test_log_loss, mm.test_brier_score "
                ...
                if not prod_row:
                    ...
                    conn.execute(text(f"UPDATE model_versions SET status = 'production' WHERE id = {cand_id}"))
                ...
                if is_better:
                    ...
                    conn.execute(text(f"UPDATE model_versions SET status = 'deprecated' WHERE id = {prod_id}"))
                    # Promote candidate
                    conn.execute(text(f"UPDATE model_versions SET status = 'production' WHERE id = {cand_id}"))
